...
app.post('/chargeForNapos', async (request, response) => {
  const requestBody = request.body;
  console.log(requestBody);
  try {
    const locationId =  process.env.LOCATION_ID;
    const createOrderRequest = getOrderRequest(locationId, requestBody.charge);
...
function getOrderRequest(locationId, cost) {
  return {
    idempotencyKey: crypto.randomBytes(12).toString('hex'),
    order: {
